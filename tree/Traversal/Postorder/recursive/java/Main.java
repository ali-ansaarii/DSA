...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean timePostorder = false;

        for (String arg : args) {
            if ("--time-postorder".equals(arg)) {
                timePostorder = true;
            } else {
                inputPath = arg;
            }
        ...
        List<Integer> order = PostorderTraversal.PostorderTraversal(leftChildren, rightChildren, root);
        double traversalDurationMs = (System.nanoTime() - traversalStart) / 1_000_000.0;

        if (timePostorder) {
            System.out.println("Visited nodes: " + order.size());
            System.out.printf(Locale.US, "PostorderTraversal call time (ms): %.3f%n", traversalDurationMs);
        } else {
            System.out.print("Postorder traversal order:");
            for (int node : order) {
