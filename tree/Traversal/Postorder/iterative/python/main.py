...
from PostorderTraversal import PostorderTraversal
...
def main() -> int:
    ...
    order = PostorderTraversal(left_children, right_children, root)
    traversal_duration_ms = (time.perf_counter() - traversal_start) * 1000

    if time_postorder:
        print(f"Visited nodes: {len(order)}")
        print(f"PostorderTraversal call time (ms): {traversal_duration_ms:.3f}")
    else:
        print("Postorder traversal order:", *order)
