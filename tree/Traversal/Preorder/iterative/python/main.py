...
def main() -> int:
    ...
    order = PreorderTraversal(left_children, right_children, root)
    traversal_duration_ms = (time.perf_counter() - traversal_start) * 1000

    if time_preorder:
        print(f"Visited nodes: {len(order)}")
        print(f"PreorderTraversal call time (ms): {traversal_duration_ms:.3f}")
    else:
        print("Preorder traversal order:", *order)
