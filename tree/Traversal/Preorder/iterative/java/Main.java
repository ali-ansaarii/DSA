...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean timePreorder = false;

        for (String arg : args) {
            if ("--time-preorder".equals(arg)) {
                timePreorder = true;
            } else {
                inputPath = arg;
            }
        ...
        List<Integer> order = PreorderTraversal.PreorderTraversal(leftChildren, rightChildren, root);
        double traversalDurationMs = (System.nanoTime() - traversalStart) / 1_000_000.0;

        if (timePreorder) {
            System.out.println("Visited nodes: " + order.size());
            System.out.printf(Locale.US, "PreorderTraversal call time (ms): %.3f%n", traversalDurationMs);
        } else {
            System.out.print("Preorder traversal order:");
            for (int node : order) {
