...
use preorder_iterative::PreorderTraversal;
...
fn main() {
    ...
    let order = PreorderTraversal(&left_children, &right_children, root);
    let traversal_duration_ms = traversal_start.elapsed().as_secs_f64() * 1000.0;

    if time_preorder {
        println!("Visited nodes: {}", order.len());
        println!("PreorderTraversal call time (ms): {:.3}", traversal_duration_ms);
    } else {
