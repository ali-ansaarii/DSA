# Inorder Iterative Usage

## Shared Inputs
This inorder topic keeps shared inputs in `../inputs/` so both recursive and iterative implementations use the same files:
...
Iterative inorder should continue to work because it manages depth explicitly with its own stack instead of the language call stack.
...
Both benchmarks measure only the `InorderTraversal(...)` call inside `main`, not file parsing, tree construction, or output printing.
The runners expose timing through `--time-inorder`, and the shared benchmark wrapper also reports sampled peak RSS so the memory output works on both macOS and Linux.

## Parent Inorder Commands
From `tree/Traversal/Inorder/`, you can also run the parent orchestration targets:
