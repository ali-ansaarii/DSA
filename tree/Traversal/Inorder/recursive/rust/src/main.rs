...
use inorder_recursive::InorderTraversal;
...
fn main() {
    ...
    let order = InorderTraversal(&left_children, &right_children, root);
    let traversal_duration_ms = traversal_start.elapsed().as_secs_f64() * 1000.0;

    if time_inorder {
        println!("Visited nodes: {}", order.len());
        println!("InorderTraversal call time (ms): {:.3}", traversal_duration_ms);
    } else {
