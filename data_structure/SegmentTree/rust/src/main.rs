...
use segment_tree::{process_segment_tree_queries, Query};
use std::env;
use std::fs;
use std::process;
...
fn read_input(input_path: &str) -> Result<(Vec<i64>, Vec<Query>), String> {
    ...
    for line_index in 0..q {
        ...
        match operation {
            ...
            _ => return Err(format!("Invalid operation at line {}.", operation_line)),
        }
    }
...
fn main() {
    ...
    let mut time_segment_tree = false;
    ...
    let results = match process_segment_tree_queries(&initial_values, &queries) {
        Ok(results) => results,
        Err(message) => {
            eprintln!("{message}");
            process::exit(1);
    ...
    if time_segment_tree {
        println!("Segment-tree time: {:.6} ms", elapsed.as_secs_f64() * 1000.0);
    } else {
