...
from SegmentTree import Query, QueryType, checked_i64, process_segment_tree_queries
...
def main() -> int:
    ...
    time_segment_tree = False

    for argument in sys.argv[1:]:
        if argument == "--time-segment-tree":
            time_segment_tree = True
        else:
    ...
    try:
        results = process_segment_tree_queries(initial_values, queries)
    except OverflowError:
        print("Segment tree overflowed while processing the queries.", file=sys.stderr)
        return 1
    end = time.perf_counter()

    if time_segment_tree:
        print(f"Segment-tree time: {(end - start) * 1000.0:.6f} ms")
    else:
