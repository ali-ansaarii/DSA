...
public final class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean timeFenwickTree = false;

        for (String argument : args) {
            if (argument.equals("--time-fenwick-tree")) {
        ...
        try {
            results = FenwickTree.processFenwickQueries(
                    inputData.initialValues(),
                    inputData.queries());
        } catch (ArithmeticException exception) {
            System.err.println("Fenwick tree overflowed while processing the queries.");
            System.exit(1);
            return;
        }
        ...
        if (timeFenwickTree) {
            ...
            System.out.printf("Fenwick-tree time: %.6f ms%n", elapsedMs);
        } else {
            printAnswer(results);
        }
    }
