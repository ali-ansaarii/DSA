...
void PrintResult(const PrimResult& result) {
...
int main(int argc, char* argv[]) {
    ...
    if (!ParseArguments(argc, argv, options)) {
        cerr << "Usage: " << argv[0] << " <input-file> [--time-prim]\n";
        return 1;
    }
    ...
    const PrimResult result = PrimMST(nodeCount, edges);
    const auto end = chrono::steady_clock::now();
    const chrono::duration<double, milli> elapsed = end - begin;

    if (options.benchmarkMode) {
        cout << fixed << setprecision(3);
        cout << "Prim time: " << elapsed.count() << " ms\n";
    }
