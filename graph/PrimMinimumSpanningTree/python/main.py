...
from PrimMST import Edge, PrimStatus, prim_mst
...
def parse_args(argv: list[str]) -> tuple[str, bool]:
    ...
    for argument in argv[1:]:
        if argument == "--time-prim":
            benchmark_mode = True
        elif input_path is None:
            input_path = argument
        else:
            raise ValueError("Usage: main.py <input-file> [--time-prim]")
...
def main(argv: list[str]) -> int:
    ...
    result = prim_mst(node_count, edges)
    elapsed_ms = (time.perf_counter() - start_time) * 1000.0

    if benchmark_mode:
        print(f"Prim time: {elapsed_ms:.3f} ms")

    if result.status == PrimStatus.OVERFLOW:
        print("Overflow detected while summing MST edge weights", file=sys.stderr)
        return 1

    if result.status == PrimStatus.DISCONNECTED:
