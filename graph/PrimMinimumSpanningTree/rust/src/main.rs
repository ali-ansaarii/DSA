...
use prim_mst::{prim_mst, Edge, Status};
use std::env;
use std::fs;
...
fn parse_arguments(args: &[String]) -> Result<ProgramOptions, String> {
    ...
    for argument in args.iter().skip(1) {
        if argument == "--time-prim" {
            ...
            return Err("Usage: prim_mst <input-file> [--time-prim]".to_string());
        }
    }
...
fn main() {
    ...
    let result = prim_mst(node_count, &edges);
    let elapsed_ms = begin.elapsed().as_secs_f64() * 1000.0;

    if options.benchmark_mode {
        println!("Prim time: {:.3} ms", elapsed_ms);
    }
