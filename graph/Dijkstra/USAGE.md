...
- `input.txt`: the small default weighted graph used for the basic correctness run
- `input_large.txt`: a weighted undirected 400x400 grid with `160,000` nodes and `319,200` edges for representative sparse-graph benchmarking
- `input_challenge.txt`: a weighted graph with `220,000` nodes and `439,998` edges designed to create many stale priority-queue entries
