...
- `inputs/input.txt`: the small default input used for the basic correctness run
- `inputs/input_large.txt`: a complete binary tree with `1,048,575` nodes and `1,048,574` undirected edges for representative performance benchmarking
- `inputs/input_challenge.txt`: a star graph with `300,000` nodes and `299,999` edges designed to stress BFS queue growth and memory pressure
